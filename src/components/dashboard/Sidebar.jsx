...
const navItems = [
  ...
  },
  {
    ...
    icon: (
      <svg width="17" height="17" viewBox="0 0 18 18" fill="none">
        <rect x="1.5" y="3" width="15" height="13.5" rx="2" stroke="currentColor" strokeWidth="1.6"/>
        <path d="M5.25 1.5v3M12.75 1.5v3M1.5 7.5h15" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/>
      </svg>
    ),
  },
];

export function Sidebar({ activePath = "/dashboard", collapsed = false, onToggle }) {
  ...
  return (
    <>
      ...
      <div className="sidebar-float">
        {/* Logo */}
        <div style={{ padding: "18px 14px 14px", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px" }}>
          {!collapsed && (
            <span style={{ fontWeight: 800, fontSize: "0.95rem", letterSpacing: "-0.02em", color: "white", whiteSpace: "nowrap" }}>
              Presta<span style={{ color: "#4ade80" }}>Control</span>
            </span>
          )}
          <button
            onClick={() => setCollapsed(!collapsed)}
            style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "9px", padding: "6px", cursor: "pointer", color: "rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginLeft: collapsed ? "auto" : 0 }}
          >
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
              {collapsed
                ? <path d="M2 6h8M6 2l4 4-4 4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                : <path d="M10 6H2M6 2L2 6l4 4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>}
            </svg>
          </button>
        </div>
        ...
        <nav style={{ flex: 1, padding: "12px 8px", display: "flex", flexDirection: "column", gap: "2px", overflowY: "auto" }}>
          {!collapsed && (
            <p style={{ color: "rgba(255,255,255,0.16)", fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", padding: "4px 6px 6px" }}>
              Menú
            </p>
          )}
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className={`sb-nav-item ${activePath === item.href ? "active" : ""}`}
        ...
        <div style={{ padding: "10px 8px 14px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          <button
            ...
            style={{ width: "100%", border: "1px solid transparent", background: "none", textAlign: "left" }}
          >
