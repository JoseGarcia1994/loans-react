...
export function WeeklyPaymentsTable({ payments, markAsPaid, liquidateLoan }) {
  if (!payments.length) return null;

  // Track which loan_ids already showed the liquidate button
  const shownLiquidate = new Set();
    ...
    <div style={{
      ...
      <div>
        {payments.map((payment, i) => {
          // Show liquidate button only on the first unpaid payment of each loan
          const showLiquidate = !payment.paid && !shownLiquidate.has(payment.loan_id);
          if (!payment.paid) shownLiquidate.add(payment.loan_id);
          ...
          return (
            <div
              ...
              style={{
                ...
                background: payment.paid ? "rgba(74,222,128,0.03)" : "transparent",
                transition: "background 0.15s",
              }}
              onMouseOver={(e) => { if (!payment.paid) e.currentTarget.style.background = "rgba(255,255,255,0.03)"; }}
              onMouseOut={(e) => { e.currentTarget.style.background = payment.paid ? "rgba(74,222,128,0.03)" : "transparent"; }}
            >
              {/* Client */}
              ...
              {/* Charge */}
              <span style={{ color: payment.paid ? "#4ade80" : "rgba(255,255,255,0.75)", fontWeight: 700, fontSize: "0.88rem" }}>
                {formatCurrency(payment.payment_amount)}
              </span>

              {/* Status */}
              <span style={{ color: payment.paid ? "#4ade80" : "#fb923c", fontSize: "0.82rem", fontWeight: 600 }}>
